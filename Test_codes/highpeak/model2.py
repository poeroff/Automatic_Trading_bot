...
def calculate_rsi(dataframe, column='Close', window=14):
    ...
    avg_gain = pd.Series(gain).rolling(window).mean().ewm(span=window, adjust=False).mean()
    avg_loss = pd.Series(loss).rolling(window).mean().ewm(span=window, adjust=False).mean()
...
def find_peaks(dataframe, high_column='Close', compare_window=23, min_gap=60, threshold=0.2):
    ...
    for i in range(compare_window, len(dataframe)):
        ...
        if (current_price > np.max(window_before) and 
            ...
            print(dataframe['Date'].iloc[i], dataframe['RSI'].iloc[i])
            if dataframe['RSI'].iloc[i] <= 100 and dataframe['RSI'].iloc[i] >=60 :  # RSI 80 이상 조건
